...
def run_quantization_experiments():
    ...
    # Skip BF16 if not supported
        ...
        # print(f"Initial {name} QAT Accuracy: {evaluate(model_qat, test_loader, device):.2f}%")
        
        # qat_history = []
        # for epoch in range(qat_epochs):
        #     print(f'\nQAT Epoch {epoch+1}/{qat_epochs}')
        #     train_loss, train_acc = train_epoch(model_qat, train_loader, 
        #                                     criterion, optimizer, device)
        #     test_acc = evaluate(model_qat, test_loader, device)
        #     qat_history.append(test_acc)
            
        #     print(f'Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.2f}%')
        #     print(f'Test Acc: {test_acc:.2f}%')
        #     print(f'Recovered from PTQ: {test_acc - results[f"{name} PTQ"]:.2f}%')
        ...
        # *** CHANGE THIS: Evaluate the QUANTIZED model ***
        # final_qat_acc = evaluate(model_qat_quantized, test_loader, device, 
        #                         dtype=torch.float16 if bit_width=='fp16' else 
        #                             torch.bfloat16 if bit_width=='bf16' else torch.float32)
        ...
        # results[f'{name} QAT'] = final_qat_acc
        # print(f"\nFinal {name} QAT Accuracy (quantized): {final_qat_acc:.2f}%")
        # print(f"Total Recovery: {final_qat_acc - results[f'{name} PTQ']:.2f}%")
        # print(f"Remaining Gap from Baseline: {baseline_acc - final_qat_acc:.2f}%")
        results['qat'][name] = {
